...
def train_nb():
 ...
 # === 7. Entraîner le modèle Naive Bayes ===
 model_nb = GaussianNB()
 model_nb.fit(X_train, y_train)

 # === 8. Sauvegarder le modèle et les objets de prétraitement ===
 joblib.dump(model_nb, "modele_naive_bayes.pkl")
 joblib.dump({"encoders": encoders, "scaler": scaler}, "encodeurs_scaler.pkl")  # Utilise le même si déjà fait

 print("✅ Modèle Naive Bayes entraîné et sauvegardé avec succès !")
 ...
 print(classification_report(y_test, y_pred_nb, target_names=["Non", "Oui"]))

 #  Affichage de la matrice de confusion
 cm_nb = confusion_matrix(y_test, y_pred_nb)
 disp_nb = ConfusionMatrixDisplay(confusion_matrix=cm_nb, display_labels=["Non", "Oui"])
